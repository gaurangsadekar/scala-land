 ...
 */
// annotating with JUnitRunner allows the test suite to be run as a JUnit suite
// removing it will make it a Scala Test Application
// extending FunSuite is important, as that is the class that is required to tell
// the compiler that this class is a testbed
...
class ListsSuite extends FunSuite {
  ...
  test("max of a few -ve numbers") {
    assert(max(List(-3, -7, -2)) === -2)
  }

  test("max of list of repeated numbers") {
    assert(max(List(-3, -7, -2, 1, 1, 1, 5)) === 5)
  }
  test("check if no such element exception is thrown correctly") {
